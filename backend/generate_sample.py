...
def generate_samples():
    ...
    # Add missing values
    df_messy.loc[5:15, 'quiz_1'] = np.nan
    df_messy.loc[20:25, 'class'] = np.nan
    # Add out of bounds
    df_messy.loc[30:35, 'Attendance_Percent'] = "150%"
    df_messy.loc[40:42, 'Attendance_Percent'] = "-10%"
    # Add duplicates
    df_messy = pd.concat([df_messy, df_messy.iloc[0:5]], ignore_index=True)
    
    df_messy.to_csv("sample_students_messy.csv", index=False)
    print("Generated sample_students.csv and sample_students_messy.csv")
